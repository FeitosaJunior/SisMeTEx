...
def graf_meteo(session):
    ...
    temperature_data = df_cort['Indoor Temperature(°C)'].tolist()
    freezing_data = df_cort['DewPoint(°C)'].tolist()
    precipitation_data = df_cort['Hour Rainfall(mm)'].tolist()
    wind_speed_data = df_cort['Wind Speed(m/s)'].tolist()
    wind_direction_data = df_cort['Wind Direction Degrees from North'].tolist()
    gust_data = df_cort['Gust(m/s)'].tolist()
    humidity_data = df_cort['Indoor Humidity(%)'].tolist()
    pressure_data = df_cort['Absolute Pressure(hpa)'].tolist()
